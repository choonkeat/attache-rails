    ...
    module Model
      ...
      module ClassMethods
        ...
        def has_one_attache(name)
          ...
          after_commit                        ->                          { return unless self.previous_changes[name]; file = self.send(name).try(:[], 'path'); self.attaches_backup!([file]) if file.present? }, on: [:create, :update]
        end
        ...
        def has_many_attaches(name)
          ...
          after_update                        ->                          { self.attaches_discarded ||= []; attache_mark_for_discarding(self.send("#{name}_was"), self.send("#{name}"), self.attaches_discarded) }
          after_destroy                       ->                          { self.attaches_discarded ||= []; attache_mark_for_discarding(self.send("#{name}_was"), [], self.attaches_discarded) }
          after_commit                        ->                          { return unless self.previous_changes[name]; files = Array.wrap(self.send(name)).collect {|x| x['path']}; self.attaches_backup!(files) if files.present? }, on: [:create, :update]
        end
      end
    end
  end
